		...
		<swiper
			...
			<swiper-item v-for="(item, index) in videoList" :key="index">
				...
				<cover-view class="cover-view-right">
					<cover-image :src="item.avater" class="avater img" @tap.stop="tapAvater"></cover-image>
					<cover-image :src="item.check?'../../static/aixinRed.png':'../../static/aixin.png'" class="img" @click.stop="tapLove"></cover-image>
					<cover-image src="../../static/xiaoxi.png" class="img" @tap.stop="tapMsg"></cover-image>
					<cover-image src="../../static/share-fill.png" class="img" @tap.stop="tapShare"></cover-image>
				</cover-view>
			</swiper-item>
		</swiper>
    </view>
					...
					uni.request({
						...
						success: (res) => {
							...
							for (let item of res.data.result) {
								//限制5条
								if(item.type == 'videoSmallCard'&&videoGroup.length<8){
									videoGroup.push({
